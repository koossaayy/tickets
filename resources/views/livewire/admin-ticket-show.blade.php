        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5">{{ __('Ticket #:param_1 · Customer: :param_2 (:param_3)', ['param_1' => $ticket->id, 'param_2' => $ticket->user->name, 'param_3' => $ticket->user->email]) }}</p>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white border border-slate-200/70 shadow-sm rounded-2xl p-6 sm:p-8">
                        <h3 class="font-bold text-slate-800 text-lg">{{ __('Send Reply to Customer') }}</h3>
                        <form wire:submit="reply" class="mt-4 space-y-4">
                            <div>
                                <textarea wire:model="body" rows="5"
                                          ...
                                          placeholder="{{ __('Type your reply to customer...') }}"></textarea>
                ...
                <div class="space-y-6">
                    <div class="bg-white border border-slate-200/70 shadow-sm rounded-2xl p-6 space-y-5">
                        <h3 class="font-bold text-slate-800 text-base pb-3 border-b border-slate-100">{{ __('Manage Ticket') }}</h3>
                        ...
                        <form wire:submit="updateStatus" class="space-y-4">
                            ...
                            <div class="pt-2">
                                <x-primary-button class="w-full justify-center rounded-xl py-2.5">{{ __('Update Settings') }}</x-primary-button>
                            </div>
                        </form>
                        
                        <p class="text-[10px] text-slate-400 text-center leading-relaxed">{{ __("Customers are automatically notified by email when you update the ticket's status (except if marked as Closed).") }}</p>
                    </div>

                    <!-- Customer Info Sidebar Card -->
                    <div class="bg-slate-50 border border-slate-200 shadow-sm rounded-2xl p-6 space-y-4">
                        <h3 class="font-bold text-slate-800 text-base pb-2 border-b border-slate-200">{{ __('Customer Details') }}</h3>
                        
                        <div class="space-y-3 text-sm text-slate-700">
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">{{ __('Name') }}</label>
                                <span class="font-semibold text-slate-800">{{ $ticket->user->name }}</span>
                            </div>
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">{{ __('Email') }}</label>
                            ...
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">{{ __('Inbound Reply Token') }}</label>
                    ...
                    <div class="bg-gradient-to-tr from-slate-900 to-indigo-950 text-slate-100 shadow-lg shadow-indigo-900/10 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <p class="text-xs text-slate-300 mt-1 leading-relaxed">{{ __('This URL allows the user to securely access their ticket without entering password.') }}</p>
